...
# RAG文档配置 - 使用用户指定的文档目录
DOCUMENTS_DIR = r'D:\shengwu\lung-nodule-app-1\datast'
        ...
        for idx, record in enumerate(records, 1):
            ...
            if metrics:
                ...
                if metrics.get('dice') is not None:
                    prompt += f"  - Dice（Dice系数）: {safe_format_metric(metrics['dice'])}\n"
                if metrics.get('iou') is not None:
                    prompt += f"  - IoU（交并比）: {safe_format_metric(metrics['iou'])}\n"
                if metrics.get('hd95') is not None:
                    prompt += f"  - HD95（95%豪斯多夫距离）: {safe_format_metric(metrics['hd95'])}\n"
                if metrics.get('precision') is not None:
                    prompt += f"  - Precision（精确率）: {safe_format_metric(metrics['precision'])}\n"
                if metrics.get('recall') is not None:
                    prompt += f"  - Recall（召回率）: {safe_format_metric(metrics['recall'])}\n"
        prompt += "\n"
...
def build_case_comparison_prompt(patient_name, records, description):
    ...
    if len(records) >= 2:
        prompt += "\n【指标变化统计】\n"
        metric_labels = ['accuracy', 'dice', 'iou', 'hd95', 'precision', 'recall']
        metric_names = ['Accuracy', 'Dice', 'IoU', 'HD95', 'Precision', 'Recall']
...
1. 影像概述
   用简洁生动的语言介绍：这是什么部位的影像？属于什么类型的检查？这张切片在整体检查中的位置是什么？让患者对检查有个整体认识。

2. 病灶可能性评估
   基于AI检测结果，评估是否存在异常。用"可能性较高/中等/较低"等表述，避免绝对化。解释为什么会这样判断，让患者理解依据。

3. 影像特征描述
   详细描述切片中可见的组织结构。如有异常区域，描述其：位置（具体在器官的哪个部位）、大小（可用日常物品比喻）、形态（圆形/不规则等）、边界（清晰/模糊）、密度/信号特点等。如无明显异常，描述正常组织的表现。

4. 临床意义解读
   将影像发现与患者的健康状况联系起来。用通俗的语言解释：这可能意味着什么？是轻微的变化还是需要关注的信号？对患者日常生活有什么影响？

5. 鉴别诊断思路
   如存在异常，分析可能是什么情况：是良性病变还是恶性可能？有哪些疾病需要考虑？各自的特点是什么？如无明显异常，说明正常表现排除了哪些问题。

6. 生活调理建议
   根据分析结果，给出具体可操作的建议：饮食上吃什么、避免什么？生活习惯上如何调整？心理上如何保持平和？运动方面有什么建议？

7. 就医指导
   明确告知患者：是否需要进一步检查？建议挂什么科室？大概多久复查一次？出现什么症状需要立即就医？

8. 温馨提示
   用温暖的语言提醒：AI分析只是辅助参考，最终诊断需要医生结合完整资料判断。鼓励患者积极面对，不要过度焦虑，也不要忽视问题。

9. 健康小知识
   分享与本次检查相关的医学常识：这个器官有什么功能？常见的相关疾病有哪些？如何预防？早期筛查为什么重要？
...
- 字数要求：报告应充实丰富，总字数建议在800-1500字之间
